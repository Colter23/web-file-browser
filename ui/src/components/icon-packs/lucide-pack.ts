import type {Component} from "vue";
import {
  Archive,
  ArchiveRestore,
  ArrowLeft,
  ArrowRight,
  ArrowDownAZ,
  ArrowDownWideNarrow,
  ArrowUp,
  ArrowUpAZ,
  ArrowUpNarrowWide,
  Box,
  Braces,
  BrushCleaning,
  CalendarClock,
  Check,
  ChevronDown,
  ChevronLeft,
  ChevronRight,
  ChevronUp,
  CirclePause,
  ClipboardCopy,
  ClipboardPaste,
  Columns3,
  Copy,
  Download,
  Eraser,
  Expand,
  Eye,
  File,
  FileArchive,
  FileBox,
  FileChartColumn,
  FileCode,
  FileCog,
  FileDigit,
  FileImage,
  FileMusic,
  FilePlus,
  FileQuestionMark,
  FileSpreadsheet,
  FileText,
  FileType,
  Folder,
  FolderOpen,
  FolderPlus,
  GalleryHorizontal,
  GalleryThumbnails,
  Grid2X2,
  GripVertical,
  History,
  House,
  Image,
  Info,
  LayoutGrid,
  List,
  ListChecks,
  ListMusic,
  Lock,
  LogOut,
  Maximize2,
  Menu,
  Monitor,
  MoreHorizontal,
  Palette,
  MousePointer2,
  Minimize2,
  Moon,
  Music,
  PanelRight,
  PanelRightClose,
  Pause,
  Pencil,
  Play,
  Plus,
  RefreshCw,
  Repeat,
  Repeat1,
  Rows3,
  RotateCcw,
  RotateCw,
  Save,
  Scissors,
  Search,
  Settings,
  Share2,
  Shrink,
  Shuffle,
  SquareArrowOutUpRight,
  SquarePen,
  Star,
  Sun,
  Terminal,
  TriangleAlert,
  Trash2,
  Upload,
  User,
  Video,
  Volume2,
  VolumeX,
  Wrench,
  X,
  ZoomIn,
  ZoomOut
} from "lucide-vue-next";
import type {AppIconPack} from "./types.ts";

const lucideIcons: Record<string, Component> = {
  "action.upload": Upload,
  "action.new-file": FilePlus,
  "action.new-folder": FolderPlus,
  "action.open": FolderOpen,
  "action.open-new-tab": SquareArrowOutUpRight,
  "action.cut": Scissors,
  "action.copy": Copy,
  "action.copy-path": ClipboardCopy,
  "action.paste": ClipboardPaste,
  "action.download": Download,
  "action.preview": Eye,
  "action.archive": Archive,
  "action.extract": ArchiveRestore,
  "action.restore": ArchiveRestore,
  "action.rename": Pencil,
  "action.delete": Trash2,
  "action.trash": Trash2,
  "action.edit": SquarePen,
  "action.save": Save,
  "action.close": X,
  "action.add": Plus,
  "action.check": Check,
  "action.refresh": RefreshCw,
  "action.recent": History,
  "action.clean": BrushCleaning,
  "action.settings": Settings,
  "action.appearance": Palette,
  "action.main-menu": Menu,
  "action.more": MoreHorizontal,
  "action.logout": LogOut,
  "action.search": Search,
  "action.share": Share2,
  "action.favorite": Star,
  "action.favorite-filled": Star,
  "action.warning": TriangleAlert,
  "action.select-all": ListChecks,
  "action.invert-selection": MousePointer2,
  "action.clear-selection": Eraser,
  "action.properties": Info,
  "action.fullscreen": Maximize2,
  "action.exit-fullscreen": Minimize2,
  "action.player-large": Maximize2,
  "action.player-small": PanelRightClose,
  "action.player-mini": Minimize2,
  "action.play": Play,
  "action.pause": Pause,
  "action.volume": Volume2,
  "action.volume-muted": VolumeX,
  "action.previous": ChevronLeft,
  "action.next": ChevronRight,
  "action.up": ChevronUp,
  "action.down": ChevronDown,
  "action.drag-handle": GripVertical,
  "action.tools": Wrench,

  "nav.back": ArrowLeft,
  "nav.forward": ArrowRight,
  "nav.up": ArrowUp,
  "nav.refresh": RefreshCw,
  "nav.recent": History,

  "sort.name": ArrowDownAZ,
  "sort.modified": CalendarClock,
  "sort.size": FileDigit,
  "sort.asc": ArrowUpAZ,
  "sort.desc": ArrowDownAZ,
  "sort.small-large": ArrowUpNarrowWide,
  "sort.large-small": ArrowDownWideNarrow,

  "playback.sequence": CirclePause,
  "playback.repeat-one": Repeat1,
  "playback.repeat-all": Repeat,
  "playback.shuffle": Shuffle,

  "appearance.system": Monitor,
  "appearance.light": Sun,
  "appearance.dark": Moon,

  "view.details": List,
  "view.list": Rows3,
  "view.icons": LayoutGrid,
  "view.tiles": Columns3,
  "view.grid": Grid2X2,
  "view.preview-pane": PanelRight,
  "view.preview-pane-close": PanelRightClose,
  "view.image": Image,
  "view.audio": Music,
  "view.playlist": ListMusic,
  "view.video": Video,
  "view.pdf": FileText,

  "viewer.page-fullscreen": Maximize2,
  "viewer.page-fullscreen-off": Minimize2,
  "viewer.browser-fullscreen": Expand,
  "viewer.browser-fullscreen-off": Shrink,
  "viewer.filmstrip": GalleryThumbnails,
  "viewer.filmstrip-off": GalleryHorizontal,
  "viewer.zoom-in": ZoomIn,
  "viewer.zoom-out": ZoomOut,
  "viewer.rotate-left": RotateCcw,
  "viewer.rotate-right": RotateCw,

  "file.home": House,
  "file.folder": Folder,
  "file.folder-open": FolderOpen,
  "file.file": File,
  "file.image": FileImage,
  "file.text": FileText,
  "file.code": FileCode,
  "file.config": FileCog,
  "file.archive": FileArchive,
  "file.audio": FileMusic,
  "file.video": Video,
  "file.pdf": FileText,
  "file.spreadsheet": FileSpreadsheet,
  "file.document": FileText,
  "file.presentation": FileChartColumn,
  "file.executable": Terminal,
  "file.shortcut": Share2,
  "file.database": FileBox,
  "file.font": FileType,
  "file.package": Box,
  "file.markup": Braces,
  "file.unknown": FileQuestionMark,
  "file.generic": File,

  "icon-upload": Upload,
  "icon-file-add-fill": FilePlus,
  "icon-folder-add-fill": FolderPlus,
  "icon-scissors": Scissors,
  "icon-contentcut": Scissors,
  "icon-copy": Copy,
  "icon-copy1": Copy,
  "icon-paste": ClipboardPaste,
  "icon-download": Download,
  "icon-download1": Download,
  "icon-download2": Download,
  "icon-download2f": Download,
  "icon-edit-filling": SquarePen,
  "icon-rename": Pencil,
  "icon-bx-rename": Pencil,
  "icon-renamebox": SquarePen,
  "icon-delete": Trash2,
  "icon-delete1": Trash2,
  "icon-delete-fill": Trash2,
  "icon-setting": Settings,
  "icon-setting-filling": Settings,
  "icon-wrench": Wrench,
  "icon-password": Lock,
  "icon-user-fill": User,
  "icon-save-fill": Save,
  "icon-close": X,
  "icon-add": Plus,
  "icon-refresh": RefreshCw,
  "icon-back_android": ChevronLeft,
  "icon-unfold": ChevronDown,
  "icon-fenxiang": Search,
  "icon-viewgrid": LayoutGrid,
  "icon-view-list": List,
  "icon-listview": Rows3,
  "icon-homefill": House,
  "icon-home-fill": House,
  "icon-file": File,
  "icon-file-fill": File,
  "icon-file-common-filling": FileText,
  "icon-file-image-fill": FileImage,
  "icon-file-zip": FileArchive,
  "icon-file-zip-fill": FileArchive,
  "icon-folder": Folder,
  "icon-folder-fill": Folder,
  "icon-folder-open-fill": FolderOpen
};

export const lucideIconPack: AppIconPack = {
  resolve(icon) {
    const component = lucideIcons[icon];
    return component ? {kind: "component", component, className: "app-icon-lucide"} : undefined;
  }
};
